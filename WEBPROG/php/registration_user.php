...
<body>
    ...
    <?php
        ...
        if (empty($LastName) || empty($FirstName) || empty($DOB) || empty($Country) || empty($Contact) || empty($Barangay) || empty($City) || empty($Province) || empty($Email) || empty($password) || empty($RepeatPassword))  {
        ...
        if (count($errors)> 0){
                ...
                require_once "database_user.php";
                $sql ="INSERT INTO tbl_user (LAST_NAME, FIRST_NAME, DOB, AGE, COUNTRY, CONTACT_NO, BARANGAY, CITY, PROVINCE, EMAIL_ADD, PASSWORD) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
                $stmt = mysqli_stmt_init($conn);
                $preparestmt = mysqli_stmt_prepare($stmt, $sql);
            if ($preparestmt) {
                mysqli_stmt_bind_param($stmt, "ssdssssssss", $LastName, $FirstName, $DOB, $age, $Country, $Contact, $Barangay, $City, $Province, $Email, $passwordHash);
       ...
       <form action="registration_user.php" method="post">
        ...
        <div class="form-group">
            <input type="submit" class="btn btn-primary key" name="submit" value="Submit">
        </div>
        <div>
            <p>Already Have an Account? <a style = "color: #329C4E;" href="login_user.php">Login Here</a></p>
        </div>
    </form>
</div>
